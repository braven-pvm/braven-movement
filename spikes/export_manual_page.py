...
The body is the model's own skin at level of detail 5, 971 vertices, skinned by
the solver's skeleton state. Vertices travel and faces do not, so a page of
five figures costs a few hundred kilobytes rather than a few megabytes.
...
def figure(character, result: dict, phase: float) -> dict:
...
def build(character, movement_id: str, drills: dict) -> dict:
    ...
    for phase in definition.phases:
        drawn = figure(character, result, phase.at_phase)
...
def main(argv: list[str]) -> int:
    ...
    pages = [build(character, movement_id, drills) for movement_id in wanted]
